...
#define W_COND(cond, ret, ...)\
	do{\
	if(!(cond)){\
	_CPP_PRINTER_(LOG_NAMESPACE LL_WARN, "condition fail \[" #cond "]. ", ##__VA_ARGS__); \
	return ret;\
	}	\
	}while(0)

#define W_COND_F(cond, args...)	W_COND(cond, false, ##args)
#define W_COND_V(cond, args...)	W_COND(cond, , ##args)
...
#define COND_V(cond) COND(cond, )

#define L_ASSERT(cond, ...)\
	do{\
	if(!(cond)){\
	_CPP_PRINTER_(LOG_NAMESPACE LL_ERROR, "assert error, condition  \[" #cond "]. ", ##__VA_ARGS__); \
	*(int *)(nullptr) = 1;\
	}	\
	}while(0)
